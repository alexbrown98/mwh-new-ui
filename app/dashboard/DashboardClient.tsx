    ...
    lmp_dir,
    multiparous_dir,
    nulliparous_dir, status_generating_map,
    status_ready_generate_map,
    tif_dir
    ...
    export default function DashboardClient({signOut,user}: WithAuthenticatorProps) {
        ...
        const [currentStatusVerbose, setCurrentStatusVerbose] = React.useState("");
        const [canGenerateMap, setCanGenerateMap] = React.useState();
        ...
        const handleNumZonesSelector = (event) => {
            ...
            setZone2Selected(numZones === 'two' || numZones === 'three' ? 'true' : 'false');
            setZone3Selected(numZones === 'three' ? 'true' : 'false');
        };
        ...
        const handleInputChangeWalkingUnmapped = (event) => {
            setTravelSpeedWalkingUnmapped(event.target.value);
        };

        const handleInputChangeWalkingMapped = (event) => {
            setTravelSpeedWalkingMapped(event.target.value);
        };
        ...
        async function handleFacilityUpload(event) {
        ...
        },[totalDemandFile, geofenceFile, travelSpeedMotorizedMapped, travelSpeedMotorizedUnmapped,
        travelSpeedWalkingMapped, travelSpeedWalkingUnmapped, latentPhaseFileMulti, latentPhaseFileNuli, laborOnsetFileLMP])
        ...
        const checkStep1Ready = () => {
            console.debug("Checking step 1 ready.")
            if (!totalDemandFile || !geofenceFile) return false;
            if (travelSpeedType === 'single') {
                if (!travelSpeedMotorizedMapped || travelSpeedMotorizedUnmapped) {
                    return false;
                }
            }
            if (travelSpeedType === 'multiple') {
                if (!travelSpeedWalkingUnmapped || !travelSpeedWalkingMapped) {
                    return false;
                }
            }
            if (!latentPhaseFileObjectMulti) return false;
            return laborOnsetFileObjectLMP;
        ...
        React.useEffect( () => {
            ...
            if (currentStatus === status_generating_map) {
                isGenerating(true);
                setCanGenerateMap(false);
            }
        }, [currentStatus])
                    ...
                    <Box>
                                                                    ...
                                                                    <Grid item xs={8}>
                                                                        <Item>
                                                                            <Box>
                                                                                ...
                                                                                <TextField  size={'small'} id="outlined-basic" variant="outlined"
                                                                                            disabled={generating}
                                                                                            value={travelSpeedWalkingUnmapped || ''}
                                                                                            onChange={handleInputChangeWalkingUnmapped}
                                                                                />
                                                                            </Box>
                                                                        </Item>
                                                                    </Grid>
                                                                    <Grid item xs={8}>
                                                                        <Item>
                                                                            <Box>
                                                                                ...
                                                                                <TextField  size={'small'} id="outlined-basic" variant="outlined"
                                                                                            disabled={generating}
                                                                                            value={travelSpeedWalkingMapped || ''}
                                                                                            onChange={handleInputChangeWalkingMapped}
                                                                                />
                                                                            </Box>
                                                                        </Item>
                                                                    </Grid>
                        ...
                        <Box sx={{mt:5, mb:5, display: activeStep === 4 ? 'block' : 'none'}}>
                            ...
                            <StatusDetermination
                                ...
                                canGenerateMap={setCanGenerateMap}
                                totalDemandFile={totalDemandFile}
                                ...
                                travelSpeedWalkingMapped={travelSpeedWalkingMapped}
                                travelSpeedWalkingUnmapped={travelSpeedWalkingUnmapped}
                                latentPhaseFileMulti={latentPhaseFileMulti}
                                latentPhaseFileNuli={latentPhaseFileNuli}
                                laborOnsetFileLMP={laborOnsetFileLMP}
                                travelSpeedType={travelSpeedType}
                                facilityData={facilityFileJson}
                                latentType={latentPhaseType}
                            />
                            <Box sx={{width:'50%'}}>
                                <Message
                                    variation="outlined"
                                    ...
                                    heading="Status">
                                    {currentStatus === status_generating_map && (
                                        <Loader/>
                                    )}
                                    {currentStatus + currentStatusVerbose}
                                </Message>
                            </Box>
                            ...
                            <Box sx={{mt:5, mb:3}}>
                                ...
                                <Button
                                    ...
                                    variant={'contained'} color={'success'}>Generate Map
                                </Button>
                            ...
                            <Button variant="contained"
                                    ...
                                    disabled={(costAndOptimizationData === null) || (generating) || (!canGenerateMap)}
                            >
